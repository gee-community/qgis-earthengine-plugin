...
def test_raster_identify():
    ...
    with patch("ee_plugin.Map.getScale", return_value=625.7583):
...
def test_reduce_region():
    ...
    assert "elevation" in result, "Elevation not found in result"
    assert result["elevation"] > 0, "Elevation is not positive"
